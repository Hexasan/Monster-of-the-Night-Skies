		...
		public void tick() {
			...
			if(Math.abs(horizontalDistance) > 0.00001) {
				...
				if(Mth.degreesDifferenceAbs(yaw, getYRot()) < 3f) {
					...
					if(getTarget() != null && getTarget().isFallFlying() && attackPhase == AttackPhase.SWOOP)
						targetSpeed = Mth.approach(targetSpeed, fastSpeed, 0.01f * (3f / targetSpeed));
					else
						targetSpeed = Mth.approach(targetSpeed, 1f, (2f / targetSpeed));
				}
				else {
					targetSpeed = Mth.approach(targetSpeed, 0.2f, 0.01f);
				}
